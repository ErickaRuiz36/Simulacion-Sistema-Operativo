...
AUTORES: 	Edgardo Adrián Franco Martínez (C) Febrero 2017
			ERICKA RUIZ LÓPEZ Abril 2021
VERSIÓN: 1.6
...
elemento Dequeue(cola * c)
{
	...
	else
	{
		...
		//Si la cola ha quedado vacia se inicializa (c->frente=NULL, c->final=NULL, c->num_elem=0)
		if(c->num_elem==0) 
		{
			Initialize(c);
		}
...
elemento Front(cola * c)
{
	if(c->num_elem==0)
	{
		printf("\nERROR: Subdesbordamiento (Front) Se intenta acceder a elemento inexistente");
		exit(1);
	}
	return c->frente->e;
}
...
elemento Final(cola * c)
{
	if(c->num_elem==0)
	{
		printf("\nERROR: Subdesbordamiento (Final) Se intenta acceder a elemento inexistente");
		exit(1);
	}
	return c->final->e;
}
...
elemento Element(cola * c, int i)
{
	...
	{
		printf("\nERROR: Subdesbordamiento (Element) Se intenta acceder a elemento inexistente");
		exit(1);
	}
	return r;
}
...
Recibe: Recibe una cola y un elemento
Devuelve: el un entero, que es l aposicion de dicho elemento
Observaciones: En caso de no encontrar el valor devuelve un cero
*/
int Search(cola *c, elemento e){
	nodo *aux;
	aux=c->frente;
	for(int i=0; i < Size(c); i++){
		if (memcmp(&e,&c->frente->e,sizeof(e))==0){
			c->frente=aux;
			return i+1;
		}
		c->frente=c->frente->siguiente;
...
/*
void Flip(cola *c)
Descripción: Invierte los elementos de la cola
Recibe: Recibe una cola por referencia
Devuelve: No devuelve ninguno valor, solo modifica las los elementos
Observaciones: No puede aplicarse un flip si la cola esta vacia
*/
void Flip(cola *c)
{
	nodo *auxfr, *auxfin, *resp;
	elemento temp;
	
	if(c->num_elem==0){
	printf("\nERROR-e=Flip(&c): No hay elementos que flipear xd");
	exit(1);
	}
	...
	if(c->num_elem%2!=0){
		while(auxfr!=auxfin){
			temp=auxfr->e;
			auxfr->e=auxfin->e;
			auxfr=auxfr->siguiente;
			auxfin->e=temp;
			
			resp=auxfr;
			while(auxfr->siguiente!=auxfin){
				auxfr=auxfr->siguiente;
			}
			auxfin=auxfr;
			auxfr=resp;
